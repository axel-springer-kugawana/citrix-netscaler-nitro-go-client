...
func lbvserver_rewritepolicy_binding_key_to_id_qs(key LbvserverRewritepolicyBindingKey, arg string) (string, map[string]string) {
	var _ = strconv.Itoa
	var args []string
...
func lbvserver_rewritepolicy_binding_key_to_id_args(key LbvserverRewritepolicyBindingKey) (string, map[string]string) {
	return lbvserver_rewritepolicy_binding_key_to_id_qs(key, "args")
}

func lbvserver_rewritepolicy_binding_key_to_id_filter(key LbvserverRewritepolicyBindingKey) (string, map[string]string) {
	return lbvserver_rewritepolicy_binding_key_to_id_qs(key, "filter")
}
...
func (c *NitroClient) ExistsLbvserverRewritepolicyBinding(key LbvserverRewritepolicyBindingKey) (bool, error) {
	...
	id, qs := lbvserver_rewritepolicy_binding_key_to_id_filter(key)
	...
	if err := c.get("lbvserver_rewritepolicy_binding", id, qs, &results); err != nil {
		...
		if len(results.Results) > 1 {
			return false, fmt.Errorf("More than one lbvserver_rewritepolicy_binding element found")
		}
